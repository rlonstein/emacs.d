...
;;; Fixes by Phil Hollenback (philiph@pobox.com) to make gnuserv/Xemacs
;;; work correctly:
;;; 1. Call read-string with proper arguments for XEmacs.
;;; 2. Call correct gnuserv functions when saving and killing buffer.
;;; 3. Oops, didn't have #2 quite right if gnuserv wasn't running. Now fixed.
...
(eval-and-compile
  ...
  (if (boundp 'running-xemacs)
      ; Added by Phil Hollenback, 7/20/00:
      ; XEmacs (21.1, at least) has this for read-string:
...
(defun post-save-current-buffer-and-exit ()
  ...
  (if (fboundp 'server-edit)
      (server-edit)
    (if (and (boundp 'running-xemacs) (fboundp 'gnuserv-kill-buffer-function))
...
(defun post-random-spook-signature ()
  "Set the signature to whatever spook mode spits out."
  (interactive)
  (save-excursion
    (goto-char (post-kill-signature))
    (insert "-- ")
...
; Ran out of keys, pick a non-obvious one.
(define-key post-mode-map "\C-c\C-o" 'post-random-spook-signature)
...
(easy-menu-define
 ...
 '("Post"
   ...
   ["Get new random spook signature" post-random-spook-signature t]
